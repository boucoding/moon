...
bool isURL(String str, [Map options]) {
  ...
  host = shift(split);
  if (split.length > 0) {
    _portString = split.join(':');
    try {
      port = int.parse(_portString, radix: 10);
    } catch (e) {
      return false;
    }
    if (!RegExp(r'^[0-9]+$').hasMatch(_portString) || port <= 0 || port > 65535) {
      return false;
    }
  }
...
bool isFQDN(str, [options]) {
  Map _defaultFqdnOptions = {'require_tld': true, 'allow_underscores': false};
...
bool isAfter(String str, [date]) {
  ...
  try {
    _stringDate = DateTime.parse(str);
  } catch (e) {
    return false;
  }

  return _stringDate.isAfter(date);
}
...
bool isBefore(String str, [date]) {
  ...
  return _stringDate.isBefore(date);
}
